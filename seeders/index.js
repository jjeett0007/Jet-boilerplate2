...
async function seedUser () {
  try {
    ...
    userToBeInserted = {
      'password':'xJ_9a9ISz4tEbjw',
      'isDeleted':false,
      'username':'Dexter_Swaniawski',
      'email':'Ellsworth.Anderson@hotmail.com',
      'isActive':true,
      'userType':authConstant.USER_TYPES.User
    };
    userToBeInserted.password = await  bcrypt.hash(userToBeInserted.password, 8);
    let user = await dbService.updateOne(User, { 'username':'Dexter_Swaniawski' }, userToBeInserted,  { upsert: true });
    userToBeInserted = {
      'password':'e3CfBDSQDKbL8ut',
      'isDeleted':false,
      'username':'Janice_Swift53',
      'email':'Gussie.Daugherty68@hotmail.com',
      'isActive':true,
      'userType':authConstant.USER_TYPES.Admin
    };
    userToBeInserted.password = await  bcrypt.hash(userToBeInserted.password, 8);
    let admin = await dbService.updateOne(User, { 'username':'Janice_Swift53' }, userToBeInserted,  { upsert: true });
